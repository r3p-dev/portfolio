...
export async function allBlogs(locale: Locale = defaultLocale): Promise<Blog[]> {
  return localised(await getCollection('blogs', isPublished), locale).sort(byNewest)
}

export async function allProjects(locale: Locale = defaultLocale): Promise<Project[]> {
  return localised(await getCollection('projects', isPublished), locale).sort(byNewest)
}
...
export async function allTags(locale: Locale = defaultLocale) {
  const [blogs, projects] = await Promise.all([allBlogs(locale), allProjects(locale)])
...
export async function taggedEntries(
  ...
  const [projects, blogs] = await Promise.all([allProjects(locale), allBlogs(locale)])

  function flatten(entries: (Blog | Project)[], type: TaggedEntry['type']): TaggedEntry[] {
    return entries
