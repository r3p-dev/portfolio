...
function migrate(opened: DatabaseSync) {
  const existing = new Set(
    (opened.prepare('PRAGMA table_info(entries)').all() as { name: string }[]).map(
      (column) => column.name,
    ),
  )
...
export function entryById(id: number) {
  return database().prepare(`SELECT ${COLUMNS} FROM entries WHERE id = ?`).get(id) as
    Entry | undefined
}
...
export function updateEntry(id: number, owner: string, name: string, message: string) {
...
export function deleteEntry(id: number, owner: string) {
  const result = database()
    .prepare('DELETE FROM entries WHERE id = ? AND owner IS NOT NULL AND owner = ?')
    .run(id, owner)
